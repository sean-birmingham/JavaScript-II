...
firstNamesAllCaps = runners.map(firstNameCaps => firstNameCaps.first_name.toUpperCase())
...
// Problem 1
//All runners' last names need to be sorted into alphabetical order

let runnersLastNames = [];
let runnersLastNamesSorted = [];

runnersLastNames = runners.map(runnerLastName => runnerLastName.last_name);
runnersLastNamesSorted = runnersLastNames.sort();

console.log(runnersLastNamesSorted);

// Problem 2
//Average of runners' donations 

let average = 0;

average = runners.reduce((a, b) => a + b.donation, 0) / runners.length;
...
// Problem 3
// Filtered array of runners who donated less than $100

let runnerDonations = [];

runnerDonations = runners.filter(ticketPrice => ticketPrice.donation < 100);
